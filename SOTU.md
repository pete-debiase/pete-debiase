# Pete's SWE Adventures
This is a short map of my projects, work, and studies as a software engineer 😊 .

<!-- When I decided to transition into tech in 2021, I had a few years of patchwork SWE experience but wanted to get more serious. My goals were to:

1. Officially reboot as a software engineer.
2. Eliminate skill gaps to maximize potential.
3. Document/version control **_everything_**.

So far it's going great - I've never been more excited to wake up every day 😊 . -->

<!-- ### Contents -->
<!-- MarkdownTOC levels="1,2,3" -->

- [Personal Projects](#personal-projects)
    + [Human-Computer Interaction](#human-computer-interaction)
    + [Natural Language Processing + Computer Vision](#natural-language-processing--computer-vision)
    + [Honorable Mention](#honorable-mention)
- [Studies](#studies)
    + [Personal](#personal)
    + [University](#university)
- [Work](#work)
- [Education / Certifications](#education--certifications)

<!-- /MarkdownTOC -->
<!-- ───────────────────────────────────────────────────────────────────────────── -->

## Personal Projects
### Human-Computer Interaction


<table>
    <tr>
        <td><a href="https://github.com/pete-debiase/SEL4412">SEL4412</a></td>
        <td>My quest to create the perfect work environment and become one with the machine.</td>
        <td>2016 - ∞</td>
    </tr>
    <tr>
        <td><a href="https://github.com/pete-debiase/AODox">AODox</a></td>
        <td>Fully custom mechanical keyboard optimized for speed/ergonomics.</td>
        <td>2018 - ∞</td>
    </tr>
    <tr>
        <td><a href="https://github.com/pete-debiase/Sparrowhawk">Sparrowhawk</a></td>
        <td>Stenography for the 21st century and beyond.</td>
        <td>2017 - ∞</td>
    </tr>
</table>

### Natural Language Processing + Computer Vision
<table>
    <tr>
        <td><a href="https://github.com/pete-debiase/Manten-Kanken">Manken Kanken (満点漢検)</a></td>
        <td>Ace the Japan Kanji Aptitude Test in style with SRS.</td>
        <td>On hold</td>
    </tr>
    <tr>
        <td><a href="https://github.com/pete-debiase/Penji-Jiten-Pro">Penji Jiten Pro (ペン字辞典プロ)</a></td>
        <td>The world's biggest online penji dictionary.</td>
        <td>On hold</td>
    </tr>
</table>

### Honorable Mention
<table>
    <tr>
        <td><a href="https://github.com/pete-debiase/Comprog">Comprog</a></td>
        <td>Adventures in competitive programming.</td>
        <td>2022 - ∞</td>
    </tr>
</table>

## Studies
### Personal
#### General
<table>
    <tr>
        <td><a href="https://github.com/pete-debiase/YouTube-Watchlist">YouTube Watchlist</a></td>
        <td>Casual extracurricular SWE studies.</td>
        <td>2022 - ∞</td>
    </tr>
    <tr>
        <td><a href="https://github.com/pete-debiase/100-Days-of-Code-Python">100 Days of Code</a></td>
        <td>The Complete Python Pro Bootcamp for 2022 (Udemy).</td>
        <td>2021 - ?</td>
    </tr>
    <tr>
        <td><a href="https://github.com/pete-debiase/Advanced-Python-OOP">Advanced Python OOP</a></td>
        <td>Python OOP with 10 Real-World Programs (Udemy).</td>
        <td>2021 - ?</td>
    </tr>
</table>

#### Data Structures and Algorithms
<table>
    <tr>
        <td><a href="https://github.com/pete-debiase/Complete-DSA-Course-Python">DSA Python</a></td>
        <td>The Complete Data Structures and Algorithms Course in Python (Udemy).</td>
        <td>2021 - ?</td>
    </tr>
</table>

### University
<table>
    <tr>
        <td><a href="https://github.com/pete-debiase/OSU-MATH-208">UND MATH 208</a></td>
        <td>Discrete mathematics (private repo).</td>
        <td>2022</td>
    </tr>
    <tr>
        <td><a href="https://github.com/pete-debiase/OSU-CS-162">OSU CS 162</a></td>
        <td>Introduction to Computer Science II (private repo).</td>
        <td>2022</td>
    </tr>
</table>

## Work
See my [LinkedIn](https://www.linkedin.com/in/pete-debiase/) for more details!

|         Job         |     Company     |                 Description                  |      Dates / Location       |
|:-------------------:|:---------------:|:--------------------------------------------:|:---------------------------:|
|  Software Engineer  |   Viscell LLC   |  Distributed Java apps for hemostasis care.  |  2020 - 2021 / Boulder, CO  |
| Software Translator |    Nintendo     | Localize Japanese SDKs for 3DS/Wii U/Switch. |  2012 - 2020 / Redmond, WA  |
|   US Patent Agent   | PAD Engineering | Translate Japanese software/semicon patents. | 2010 - 2020 / Las Vegas, NV |

## Education / Certifications
<table>
    <tr>
        <th align="center">Degree / Certification</th>
        <th align="center">Organization</th>
        <th align="center">Date</th>
    </tr>
    <tr>
        <td>Bachelor of Science, Computer Science</td>
        <td>Oregon State University</td>
        <td align="center">Exp. 2023</td>
    </tr>
    <tr>
        <td>Master of Engineering, Focus in Technical Japanese, 4.0</td>
        <td>University of Wisconsin-Madison</td>
        <td align="center">2014</td>
    </tr>
    <tr>
        <td>Bachelor of Science, Aerospace Engineering, 3.9 (Summa)</td>
        <td>University of Colorado Boulder</td>
        <td align="center">2011</td>
    </tr>
    <tr>
        <td>US Patent Agent (Reg. No. 77,431)</td>
        <td>US Patent and Trademark Office</td>
        <td align="center">2018</td>
    </tr>
    <tr>
        <td>Japanese Language Proficiency Test – N1</td>
        <td>The Japan Foundation (国際交流基金)</td>
        <td align="center">2013</td>
    </tr>
    <tr>
        <td>Japanese Language and Culture Immersion Program</td>
        <td>Sophia University Tokyo (上智大学)</td>
        <td align="center">2010</td>
    </tr>
</table>
